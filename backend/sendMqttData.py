...
class mqtt_data():
    ...
    def __init__(self):
        self.gensets = []
        all_genset = db.get_all_genset_data()
        self.gensets = self.process_genset(all_genset)

    def process_genset(self, data_genset):
        json_data = json.loads(data_genset)
        result = []
        for data in json_data[0] :
            result.append(data.genset_ASI_genset_id)
    ...
    def cek_genset(self, data):
        ...
        try:            
            ...
            # if info is None:                
            if data['Id'] in self.gensets:
                datas['Id'] = data['Id']
                datas['name'] = None
                datas['detail'] = None
                datas['tank_capacity'] = "0"
                datas['displacement'] = None
                datas['starting_system'] = None
                datas['design_features'] = None
                datas['full_load_consumption'] = "0"
                datas['three_quarter_load_consumption'] = "0"
                datas['half_load_consumption'] = "0"
                datas['quarter_load_consumption'] = "0"
                datas['bore_stroke'] = None
                datas['continuous_rated_output'] = "0"
    ...
    def __init__(self):
        ...
        try:
            client = mqtt.Client("Pusher_1", transport="tcp")  # create new instance
